...
public class CookieModule implements HTTPClientModule
{
	     ...
	     cookie_jar.isFile()  &&  cookie_jar.canWrite()))
	{
	    ...
	    Enumeration enum2 = Util.getList(cookie_cntxt_list,
	    ...
	    {
		Cookie cookie = (Cookie) enum2.nextElement();
		if (!cookie.discard())
	...
	synchronized (cookie_list)
	{
	    ...
	    Enumeration enum2 = cookie_list.elements();
	    while (enum2.hasMoreElements())
		cookies[idx++] = (Cookie) enum2.nextElement();
